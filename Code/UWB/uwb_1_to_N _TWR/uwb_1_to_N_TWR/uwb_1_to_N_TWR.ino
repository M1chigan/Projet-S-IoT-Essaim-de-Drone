...
uint32_t getResponderDelayUs(uint8_t my_id) {
    uint8_t slot_index = my_id - 2; 
...
void processIncomingPacket() {
    ...
    if (f.msg_type == MSG_TYPE_POLL) {
        ...
        // Use the staggered delay calculated from this drone's ID
        sendResponse(f.src_id, getResponderDelayUs(SELF_DRONE_ID));
    }
    
    else if (f.msg_type == MSG_TYPE_RESP && SELF_DRONE_ID == 0x01) {
...
void loop() {
    ...
    if (SELF_DRONE_ID == 0x01) {
        if (now_ms - last_poll_time >= POLL_INTERVAL_MS) {
