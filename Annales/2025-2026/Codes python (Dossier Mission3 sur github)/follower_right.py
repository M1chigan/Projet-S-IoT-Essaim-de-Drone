...
d2=0
change = False
# Thread d'acquisition des mesures UWB
def measure_thread():
    global d1
    global d2
    global change
...
try:
    while True:
        ...
        if not data:
            break
        data = data.decode("utf-8")
        #Le drone de droite répond aux requêtes de distance avec sa distance au drone de gauche
        if "REQ" in data:
            client_socket.send(f"D:{d1}".encode("utf-8")) 
        else:
            try:
